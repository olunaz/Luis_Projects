...
class Cuadre_Cuentas{
    ...
    Init(Cuenta, Propiedad,Moneda,Descripcion,Tipo,Fisico,Desmate,FisicoInterna,DesmateInterna,NombreCustodio){
...
ToPrint2(vSheet, arrayReporte, xl){
    ...
    for i, vDataReporte in arrayReporte {
        ...
        vSheet.Cells(i+1,1) := vDataReporte.Cuenta
        vSheet.Cells(i+1,2) := vDataReporte.NombreCustodio
        vSheet.Cells(i+1,3) := vDataReporte.Propiedad
        vSheet.Cells(i+1,4) := vDataReporte.Moneda
        vSheet.Cells(i+1,5) := vDataReporte.Descripcion
        vSheet.Cells(i+1,6) := vDataReporte.Tipo
        vSheet.Cells(i+1,7) := vDataReporte.Fisico
        vSheet.Cells(i+1,8) := vDataReporte.Desmate			
        vSheet.Cells(i+1,9) := vDataReporte.FisicoInterna
        vSheet.Cells(i+1,10) := vDataReporte.DesmateInterna
